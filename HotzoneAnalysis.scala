...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val ResultDF = spark.sql("SELECT joinResult.rectangle as rectangle, count(1) from joinResult group by joinResult.rectangle order by count(1)")
    ResultDF.createOrReplaceTempView("ResultDF")
    val result = ResultDF.coalesce(1)
